...
class GoogleDriveService {
  ...
  Future<drive.DriveApi> _getDriveApi({bool promptIfNecessary = true}) async {
    await _ensureInitialized();

    Map<String, String>? headers;

    if (promptIfNecessary) {
      // Always try silent first; fall back to interactive authenticate + authorize.
  ...
  Future<String> uploadBackup(File backupFile) async {
    ...
    final result = await api.files.create(meta, uploadMedia: media, $fields: 'id');
    return result.id!;
  }
